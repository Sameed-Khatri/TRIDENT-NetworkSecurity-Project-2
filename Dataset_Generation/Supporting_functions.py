...
def get_flow_info(flows, flow):
    ...
    byte_counts = [e['byte_count'] for e in flows[flow]]
    # Convert Scapy timestamps to standard Python floats
    timestamps = sorted([float(e['ts']) for e in flows[flow]])

    bytes_sum = sum(byte_counts)
    if len(timestamps) > 1:
        idle_time = timestamps[-1] - timestamps[-2]
    else:
        idle_time = timestamps[-1]

    max_timestamp = max(timestamps)
    min_timestamp = min(timestamps)
    sum_timestamps = sum(timestamps)
    average_timestamp = sum_timestamps / len(timestamps)
    std_timestamp = float(np.std(timestamps))  # Convert numpy type to float

    flow_duration = timestamps[-1] - timestamps[0]  # Total duration of the flow
    active_time = flow_duration

    return bytes_sum, flow_duration, max_timestamp, min_timestamp, sum_timestamps, average_timestamp, std_timestamp, idle_time, active_time
