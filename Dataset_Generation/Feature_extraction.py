...
class Feature_extraction():
    ...
    def pcap_evaluation(self,pcap_file,csv_file_name):
        ...
        # Convert 'IAT' to a numeric type if it's not already
        processed_df['IAT'] = pd.to_numeric(processed_df['IAT'], errors='coerce')
        ...
        while last_row < len(processed_df):
            ...
            numeric_cols = sliced_df.select_dtypes(include=['number'])
            mean_df = pd.DataFrame(numeric_cols.mean()).T  # Calculate mean only for numeric columns

            df_summary_list.append(mean_df)
            last_row += n_rows

            # Concatenate after the loop
            final_processed_df = pd.concat(df_summary_list).reset_index(drop=True)
            final_processed_df.to_csv(csv_file_name + ".csv", index=False)
        return True
